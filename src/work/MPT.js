...
      //如果是 extension 节点，则对未尾字符和子节点的哈希值进行拼接计算
...
      const childHash = this.getTrieHash(node.children[key])
    ...
    //如果是branch节点，则对每个子节点的哈希值进行拼接计算
    const childrenList = []
      for (const key of object.keys(node.children).sort()){
        const childHash = this._getTrieHash(node.children[key])
        const encodedKey = this.encodedKey(key)
        childrenList.push(Buffer.concat([valueHash,childHash]))
      }
      childrenHash = Buffer.concat(childrenList)
    }  
  }
  //将字符串类型的key转换为Buffer类型
  encodekey (key){
  ...
  validate (address,balance){
    ...
    return rootHash === expectedBalance && actualBalance === expectedBalance
  }
}
export default MPT